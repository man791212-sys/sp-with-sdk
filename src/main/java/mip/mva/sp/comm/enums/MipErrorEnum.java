...
public enum MipErrorEnum {
	...
	SP_MISMATCHING_DATA(10151, "mismatching data"), // data 불일치
	// 절차
	...
	SP_AUTH_ERROR(19003, "SP authentication error"), // SP 인증 오류
	...
	MB_INVALID_RESULT(20101, "invalid result"), // result가 true/false가 아님
	...
	MB_UNKNOWN_DISCONNECT(20203, "unknown disconnect"), // 앱의 강제 종료 등으로 인한 서버 끊김 
	// Profile
	...
	MB_FAIL_TO_GET_DID_DOCUMENT(20307, "fail to get DID document"), // DID document 조회 실패
	MB_FAIL_TO_GET_VC_STATUS(20308, "fail to get VC Status"), // VC 상태 업데이트 실패
	// 영지식
	...
	// API
	MB_FAIL_TO_GET_PROFILE(20501, "fail to get Profile"), // 프로필 조회 실패
	MB_FAIL_TO_GET_IMAGE(20502, "fail to get Image"), // 이미지 조회 실패
	MB_FAIL_TO_GET_VP(20503, "fail to get VP"), // VP 제출 실패
	MB_VP_PROOF_FAIL(20601, "VP proof fail"), // VP 생성 실패
	// 기타
	MB_CANCEL_BY_USER(20901, "cancel by user"), // 사용자에 의한 취소
	MB_TRANSMIT_SIZE_EXCEEPD(20903, "transmit size exceeded"), // 전송 사이즈 추가
	MB_USER_AUTH_FAIL(90001, "user authentication fail"), // 사용자 인증 실패
	MB_INVALID_APP_VERSION(90002, "invalid app version"), // 사용할 수 없는 앱 버전
