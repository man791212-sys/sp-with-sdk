...
public class MipDidVpServiceImpl implements MipDidVpService {
	...
	public Boolean verifyVp(TrxInfoVO trxInfo, VP vp) throws SpException {
		...
		try {
			...
			if (ObjectUtils.isEmpty(vcResult) || !Objects.equals("1", vcResult.getStatus())) {
				return result;
			}
			// VP 검증 End
			...
			if (Objects.equals(vcStatus, VcStatusEnum.ACTIVE.getVal())) { // 활성화 상태
				LOGGER.debug("VC 활성화 상태");

				result = true;
			} else if (Objects.equals(vcStatus, VcStatusEnum.NEED_RENEW.getVal())) { // 갱신필요 상태
				String memo = resultVcStatus.getMemo();

				if (Objects.equals("주소변경", memo)) {
	...
	public Boolean reVerifyVP(VP vp) throws SpException {
		...
		return Objects.equals("1", vcResult.getStatus());
	}
