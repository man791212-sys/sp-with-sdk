...
public class MipZkpVpServiceImpl implements MipZkpVpService {
	...
	public Boolean verifyVp(TrxInfoVO trxInfo, VP vp) throws SpException {
		...
		try {
			...
			if (ObjectUtils.isEmpty(resultJson) || !resultJson.isResult()) {
				return result;
			} else {
				result = true;
			}
			...
			if (ObjectUtils.isEmpty(vpNonce) || !vp.getZkpNonce().contains(profileNonce)) {
	 ...
	 */
	@Override
	public List<Map<String, String>> getPrivacyZkp(String trxcode) throws SpException {
		...
		return verifyManager.getPrivacyZkp(svcCode, data);
	}
