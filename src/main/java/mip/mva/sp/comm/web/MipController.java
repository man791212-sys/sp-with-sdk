...
public class MipController {
	 ...
	 */
	public MipController(ConfigBean configBean, DirectService directService, TrxInfoService trxInfoService, MipDidVpService mipDidVpService, MipZkpVpService mipZkpVpService) {
	...
	public MipApiDataVO getPrivacyZkp(@RequestBody MipApiDataVO mipApiData) throws SpException {
		...
		try {
			...
			List<Map<String, String>> privacyList = mipZkpVpService.getPrivacyZkp(trxInfo.getTrxcode());
	 ...
	 * SP정보 조회
	 * 
	 * @return {"result": true, "data": "base64로 인코딩된 SP정보"}
	 * @throws SpException
	 */
	@PostMapping("/spInfo")
	public MipApiDataVO getSpInfo() throws SpException {
		LOGGER.debug("SP정보 조회!");
