...
public class QrcpmServiceImpl implements QrcpmService {
	...
	private static final CpmClient CPM_CLIENT = new CpmClient();
	 ...
	 */
	public QrcpmServiceImpl(TrxInfoService trxInfoService) {
	...
	public T520VO start(T520VO t520) throws SpException {
		...
		try {
			...
			wsInfo.setTimeout(ConfigBean.CPM_TIMEOUT);
			wsInfo.setSvcCode(svcCode);

			CPM_CLIENT.start(wsInfo);
		} catch (SpException e) {
			throw e;
		} catch (Exception e) {
