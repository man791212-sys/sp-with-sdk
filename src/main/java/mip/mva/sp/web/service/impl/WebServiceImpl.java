...
public class WebServiceImpl implements WebService {
	...
	/** 휴대폰 번호 검증 패턴 (휴대폰 번호, 국내 10~11자리) */
	private static final Pattern TELNO_PATTERN = Pattern.compile("^01[016789]\\d{7,8}$");
	 ...
	 */
	public WebServiceImpl(ConfigBean configBean, TrxInfoService trxInfoService, DirectService directService, VerifyManager verifyManager, HttpUtil httpUtil) {
	 ...
	 */
	@Override
	public T540VO pushStart(T540VO t540) throws SpException, PushException {
		...
		try {
			...
			if (!TELNO_PATTERN.matcher(telno).matches()) {
				throw new SpException(MipErrorEnum.SP_INVALID_DATA, null, "t540.telno");
			}
			...
			if (Boolean.FALSE.equals(pushInfo.getResult())) {
	...
	public T510VO qrStart(T510VO t510) throws SpException {
		...
		try {
			...
			if (Objects.equals(ModeEnum.DIRECT.getVal(), mode)) {
	...
	private M200VO directStart(TrxInfoVO trxInfo) throws SpException {
		...
		try {
			String serverDomain = configBean.getVerifyConfig().getSp().getServerDomain();
			String biImageUrl = configBean.getVerifyConfig().getSp().getBiImageUrl();
			...
			m200.setMode(mode);
			m200.setImage(biImageUrl);
			m200.setCi(isCi);
			m200.setTelno(isTelno);
			m200.setHost(serverDomain);
		} catch (SpException e) {
			throw e;
		} catch (Exception e) {
			throw new SpException(MipErrorEnum.UNKNOWN_ERROR, null, e);
	 ...
	 */
	@Override
	public T530VO app2AppStart(T530VO t530) throws SpException {
		try {
	 ...
	 */
	@Override
	public void authCmpl(String trxcode) throws SpException {
		try {
			TrxInfoVO trxInfo = trxInfoService.getTrxInfo(trxcode);
			Integer trxTimeout = configBean.getVerifyConfig().getSp().getTrxTimeout();

			if (Objects.equals(TrxStsCodeEnum.VERIFY_COM.getVal(), trxInfo.getTrxStsCode())) {
				...
				if (ObjectUtils.isEmpty(trxInfo.getVp())) {
					...
					if (Objects.equals(PresentTypeEnum.DID_VP.getVal(), presentType)) {
						...
						try {
							...
							throw new SpException(MipErrorEnum.SP_UNEXPECTED_MSG_FORMAT, trxcode, e);
						}
						...
						LOGGER.debug("영지식은 거래상태코드가 검증 완료상태이면 무조건 성공");
					} else {
				...
				if (Objects.equals(TrxStsCodeEnum.VERIFY_ERR.getVal(), trxInfo.getTrxStsCode())) {
					throw new SpException(MipErrorEnum.SP_AUTH_ERROR, trxcode);
				} else {
	...
	public String rsaEncrypt(String data, String targetDid, Boolean isOaep) throws SpException {
		return verifyManager.rsaEncrypt(data, targetDid, isOaep);
	}
	...
	public String rsaDecrypt(String data, Boolean isOaep) throws SpException {
		return verifyManager.rsaDecrypt(data, isOaep);
	}
